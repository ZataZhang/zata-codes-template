...
- 安全相关的 `validate_` 判定要同时挂在**写入点**和**消费点**（装配、启动或调用前），两处调用同一个函数，不要为兜底另写一份。写入校验只拦得住新数据，规则上线前入库的行、脚本或迁移直接写入的数据都会绕过它；消费点命中时 fail-closed，且只跳过那一条，别让一条坏数据拖垮整体
- 用户或管理员可写、最终交给第三方库或子进程的值，先确认该库会不会对它做变量展开或模板渲染，会就在上一条的两个点拒绝对应语法。例如 `langchain-mcp-adapters` 启动 stdio MCP 子进程前，会用**宿主进程**的环境变量展开 env 值里的 `${VAR}`：放行就等于让填表的人把宿主上的密钥填进子进程
