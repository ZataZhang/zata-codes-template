...
> **本原型记录的是评审当时的设计，部分已被实现取代。** 改动视图后来从「单列表 + 基线
> 选择器（工作区 / 本地分支）」改成按 `git status` 口径分三段——**已暂存 / 未暂存 /
> 未跟踪**，并移除了基线选择器与 `--diff [基线]` 参数。下面「最终画面」的改动视图截图与
> 「可交互状态」里的「切换基线」两步反映的是旧设计。**当前行为以
> `docs/guides/file-viewer.md` 与实现为准**，本页不再更新。
