...
* [ ] **BLOCKER when delivery includes a PR:** Read `references/pr-evidence-and-merge-acceptance.md`; the PR body uniquely links the PRD and says merge means acceptance, and the stable PR evidence comment presents every 9.1 human surface plus verifier/gate/tree provenance without requiring local file access
* [ ] **BLOCKER when a PR changes user-visible frontend:** The PR evidence comment shows the target prototype image and the real implementation screenshot together for every acceptance-critical state, with matching state/viewport/theme/data and truthful labels; a non-visual frontend change carries an explicit waiver
