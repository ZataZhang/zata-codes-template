...
16. **Two-Audience Evidence:** Every oracle declares its `reviewer` by audience, not by risk tier — `human` when the outcome is user-perceivable (a screen, an interaction, a generated artifact) and must ship a `presentation` (real-entry screenshot or recording, a URL to open, or the artifact file itself) for the Section 9.1 Human Review Surface; `verifier` when eyeballing adds nothing (builds, exit codes, static checks) — those the Agent self-verifies and the verifier reviews, and they reach the human only on failure. "Oracle ran green" is a machine-layer prerequisite, never a human checkbox; the human confirms decisions and perceivable outcomes. Presentation is an action, not a location: the completion message must show the surface or provide one actionable browser/local review entry point; artifacts filed in an evidence directory without being surfaced count as not delivered. A local review bundle should gather the human-facing material in one HTML page with its media and use relative links so reviewers do not have to download each item separately. The historical failure mode was a 266-line evidence report full of exit codes that fully satisfied the evidence spec while the human never learned what the delivered feature looks like.
17. **PR-Native Acceptance:** When the authorized delivery flow creates a pull request, the PR becomes the default end-of-flow human review surface: its body states the decisions and the exact "merge means acceptance" consequence, while a stable evidence comment presents Section 9.1 plus the validated evidence package. Publish this review surface before asking the user for final human acceptance. Use an in-PR browser presentation when it is accessible under the repository's privacy boundary; otherwise provide a local review bundle with a single open command. If a matching worktree exists, use it; if not, provide executable commands to fetch the implementation and evidence branches, create the worktrees, and open the page. Opening recorded evidence does not require starting the application; start it only when the reviewer needs to reproduce a live flow. Do not publish private evidence to a public host without explicit authorization. An explicitly declared PR merge — squash, merge commit, or rebase — is a traceable human acceptance event; it replaces a second confirmation in chat, never the evidence or verifier gates. For user-visible frontend changes, the review surface must pair the target prototype image with the real implementation screenshot for the same state and viewport, clearly labeling their different validation levels. Bind evidence to the verified Git tree, not only the pre-merge commit SHA, and archive only after the merged tree is proven equivalent. Read [references/pr-evidence-and-merge-acceptance.md](references/pr-evidence-and-merge-acceptance.md) whenever implementation delivery includes creating or updating a PR. A PR without the declaration, unique PRD link, published evidence, and green required gates is only a code merge and MUST NOT silently satisfy `Human-Confirmed`.
18. **Review Incidents Are Not Product Failures:** Keep a verifier/reviewer infrastructure or capability failure separate from a substantive acceptance failure. A timeout, malformed or missing verdict, unavailable tool, unsupported evidence modality, broken review environment, or demonstrably contradicted reviewer claim is `REVIEW_INCIDENT / INCONCLUSIVE`, not `BLOCKER` and never `PASS`. Retry through an independent verifier or clean environment only within the bounded review budget, preserve the raw diagnostic record, then surface the unresolved incident in a reviewable blocked Draft PR or equivalent human review surface. It must not prevent publishing reviewable work, but it must prevent acceptance, automatic merge, and archival until a valid verdict or explicit human adjudication exists. An executor may not self-declare a substantive finding to be a review incident merely because it disagrees with it. Read [references/validation-evidence-integrity.md](references/validation-evidence-integrity.md) for classification and [references/pr-evidence-and-merge-acceptance.md](references/pr-evidence-and-merge-acceptance.md) for PR behavior.
...
When the requested or repository-standard delivery flow includes a pull request, read [references/pr-evidence-and-merge-acceptance.md](references/pr-evidence-and-merge-acceptance.md) before composing the PR. The PR body and evidence comment must make the same decisions and presentations visible as Section 2 and Section 9.1; do not make the reviewer return to a local-only evidence path. If repository automation supports post-merge reconciliation, merging the declared PR is the human acceptance event and the automation records it, verifies merged-tree equivalence, updates the PRD, and archives it. Without that automation, the merge still supplies an acceptance record, but the PRD remains pending until an agent safely reconciles the merged tree and records the event; never pre-check human items merely because a PR is expected to merge.
...
Always include a **Change Impact Tree** and at least one **flow/architecture diagram** (content rules A, B). For every user-visible frontend change, include or identify a reviewable target prototype artifact that can be rendered to an image and paired with the final implementation screenshot in the PR; contained changes may use a focused low-fidelity state, while UI-heavy, multi-step, or layout-sensitive work needs the repository's interactive/high-fidelity prototype path. Pure frontend plumbing with no visual or interaction change may state a specific prototype-image waiver. Add an **ER diagram** (rule D) only when the data model or persistent state changes.

Create or modify interactive prototype files under `docs/prototypes/` when the user explicitly asks for a prototype/wireframe/demo, when static diagrams cannot express the behavior, or when a user-visible frontend change has no current target prototype that can support the required PR comparison. Skipping a new prototype file is allowed only when an existing prototype is still the truthful target or the change has no visual/interaction effect; record which case applies. This does not waive the Frontend Impact Gate — the frontend must still be planned in the Change Impact Tree and validation.

When interactive prototype files or high-fidelity prototype images will be created or edited, discover and load the repository's dedicated prototype skill (for this template: `skills/interactive-ui-prototype/SKILL.md`) before producing artifacts. That skill owns the connected-prototype, Prototype Hub, return-entry, provenance-sidecar, and browser-validation contract. The PRD skill does not weaken those requirements. If the dedicated skill is unavailable, inspect the repository's existing Hub/index conventions and apply the same outcomes explicitly; do not treat a Markdown index or `mkdocs.yml` nav entry alone as Hub registration.
...
- When flipping to `🧍 待人工验收`, also materialize `tasks/evidence/<prd-stem>/human-review-checklist.md`: the consolidated review page for every still-open item, so the human reviews one document instead of hunting through the PRD, the evidence directory tree, and the conversation. List the open `Human-Confirmed` items and human decisions in review order, each written for human reading rather than machine convenience: **decision first** — the opening line names what the human is deciding and what breaks if judged wrong; **self-contained** — quote the PRD statement verbatim and expand it in plain language, so references serve verification, not comprehension; **stable anchors** — cite the PRD by section (`§9.2 Human-Confirmed item N`), never line numbers, which drift on every PRD edit; **progressive disclosure** — one-line verdict → quoted statement → evidence → optional rerun commands last; **human-entry evidence** — render visual artifacts inline with relative paths and quote key report lines verbatim, open commands only as an optional deep dive; **explicit reply format** — state per item (or in one header table) how to answer: agree / pick an option / list differences; **jargon glossed at first use** — project-internal terms get a one-line plain-language definition. Where an item asks the human to choose (accept a disclosed gap vs. commission a follow-up test), present the options as checkboxes. When the checklist carries inline screenshots or choice items, also materialize a self-contained interactive companion `human-review-checklist.html` in the same directory (no external dependencies): a step-by-step wizard rendering the same content with one button press per decision, answers persisted in browser localStorage, and a final page emitting the review result as copyable markdown — the markdown stays the static base, and the two must carry identical content. The checklist is a presentation surface for the review session, never a second source of truth: confirmed outcomes land in the PRD §9 and the evidence package, and the checklist is kept as the review's trace. Point the user at it with the runnable `just prd review <prd-file>` (opens the interactive HTML when present, else the checklist markdown; falls back to the evidence report when no checklist exists yet; fails loudly when neither is present).
- If delivery uses a pull request, publish the same review content into that PR as defined in [references/pr-evidence-and-merge-acceptance.md](references/pr-evidence-and-merge-acceptance.md). The local checklist remains the durable source used to construct the PR presentation; the reviewer must not need to open it separately. Keep the banner at `🧍 待人工验收` until the declared PR is actually merged and the merged-tree check passes; opening or approving a PR alone does not make the PRD archive-ready.
...
A pre-altitude orientation block placed between the orientation note and `# Part A`. In the PRD it is an unnumbered `##` heading, deliberately outside the Section 1-13 numbering: it belongs to neither altitude because the human reviewer and the executor both read it first.

It exists because Part A is optimized for *correctability* — a behavior table the reader can red-line — and nothing in it answers "what is this, in 20 seconds". `What The User Gets` states per-role value, not the capability list; a reader otherwise reaches the Problem Statement before knowing what is being built.

Rules:

- It is a **plain-language projection of Section 10 Functional Requirements**, never a second source of truth. Say so in a one-line note under the heading and point behavior acceptance at the Section 1 behavior-example table.
- Every bullet carries the FR id(s) it projects, as `**短标题**（FR-n、FR-m）`. A scope boundary with no FR of its own anchors to a section instead, for example `（§11）` for a non-goal.
- **Every FR must appear in at least one bullet's anchor list.** This is the rule that earns the block its keep: a missing or mismatched anchor is a grep-able signal that the overview and the requirements have diverged, and it catches the projection error a prose summary hides.
- Aim for 5-8 bullets of one to three lines each. Needing more is a scope signal about the PRD, not a formatting problem in the overview.
- Keep it behavioral. Deployment or configuration mechanics belong here only when an operator's own workflow depends on them; internal mechanism stays in Section 6.
- Regenerate it whenever the Functional Requirements change, including during Final Reconciliation before archive.
...
- this section is the **single human-facing acceptance artifact** ("look once at the end"), organized in two audience layers (Core Rule 16, content rule I):
  - `### 9.1 人读呈递区（Human Review Surface）` first — one table row per `reviewer: human` oracle: the plain-language outcome to look at, the presentation artifact (path filled in at delivery), and an optional ~10-second self-check the human can run; plus one explicit note naming the `reviewer: verifier` groups deliberately not shown. Sized for a single pass of a few minutes. The presentation must be **actionable, not a filename**: an absolute path plus a runnable open command (`open "<path>"` on macOS; a URL for pages, PRs, and CI runs), an inline real-render image whenever the artifact is visual (a spreadsheet, report, or page — rendered from the real file/page and labelled local-only when it is gitignored, never a rebuilt mock-up), and the ~10-second self-check must name the exact place and expected value (which sheet/column/field, old→new, which element on which page) instead of "open it and take a look". When a PR is part of delivery, every row must also be publishable in the PR evidence comment; a local-only path is an intermediate capture location, not the final presentation.
  - `### 9.2 Acceptance Evidence Package` next — the machine-facing package ordered by the Section 7 risk classification: human-confirmed and `R3` oracle results first, then `R2` results and decision reconciliation, then contract diffs and folded `R1`/`R0` gate results.
- every checkbox must be **evidence-bearing**: name the command output, observation, or artifact that proves it, not a bare claim
- a `Human-Confirmed` group whose checkbox items correspond one-to-one to the human-confirm change points in the Section 2 Human Review Map, plus one item covering the 9.1 surface review; bare "oracle X ran green" items do not belong here — oracle success is a machine-layer prerequisite verified by the Agent and the verifier
- grouped checklist headings such as `Architecture Acceptance`, `Dependency Acceptance`, `Behavior Acceptance`, `Frontend Acceptance` (when a frontend app changes), `Documentation Acceptance`, `Validation Acceptance`, and `Delivery Readiness` (the overall delivery gate formerly in Definition Of Done) when relevant; `Delivery Readiness` must include an item requiring the PR evidence presentation to carry the 9.1 surface contents when a PR exists, otherwise the completion message must carry them verbatim — artifacts filed in an evidence directory but never shown count as not delivered
...
### 7. PR Evidence And Merge Acceptance

- A PR may satisfy `Human-Confirmed` only when its body contains a unique pending PRD path and an explicit statement that merging accepts the listed decisions/outcomes and authorizes post-merge archival.
- The PR must carry a stable evidence comment containing the Section 9.1 human presentations, verifier result, required-gate summary, evidence links or inline artifacts, reproducible commands, evidence hashes, and the verified head and Git tree identifiers. Raw evidence stays out of the code diff and is published through the repository's evidence-artifact mechanism or a dedicated evidence branch.
- For every user-visible frontend change, the evidence comment must render an explicit prototype-versus-implementation pair for each acceptance-critical state: the target prototype image first and a screenshot captured from the real production entry second, matched by state, viewport, theme, and representative data. Label the prototype as design intent, never as runtime proof; label the implementation screenshot with its actual validation level. Pure non-visual frontend changes require a written waiver instead of fabricated images.
- Squash merge, merge commit, and rebase merge are equivalent acceptance events. Commit SHA equality is not required; the final merged Git tree must equal the verified tree, or the final tree must be revalidated before archival.
- Opening, approving, closing without merge, or merging a PR that lacks the declaration/evidence/gates does not satisfy `Human-Confirmed`.
- Post-merge reconciliation records PR number, merger, merged time, merge method when available, verified head/tree, final commit/tree, and gate results; it then checks the Human-Confirmed items, flips the banner, performs Final Reconciliation, and moves the PRD to `tasks/archive/`. Without a safe post-merge writer, keep the PRD pending until an agent records the same facts; never predictively check acceptance.
- The detailed presentation and safety protocol is [references/pr-evidence-and-merge-acceptance.md](references/pr-evidence-and-merge-acceptance.md). Repository automation may choose its own branch names and comment markers but must preserve these semantics.
...
* [ ] A verifier/reviewer infrastructure, protocol, environment, or capability failure is recorded as `REVIEW_INCIDENT / INCONCLUSIVE`, never as product `BLOCKER` or `PASS`; bounded retry diagnostics are preserved, reviewable work may still enter a blocked Draft PR, and merge/archive remain blocked until valid adjudication
...
* [ ] **BLOCKER when delivery includes a PR:** Read `references/pr-evidence-and-merge-acceptance.md`; the PR body uniquely links the PRD and states the merge-as-acceptance consequence; publish the 9.1 human surface, verifier/gate summary, and verified-tree provenance before asking the user to review or accept
* [ ] **BLOCKER when delivery uses a local review bundle:** `human-review-checklist.html` (or the evidence worktree's `index.html`) opens all human-facing evidence using relative assets; if the matching worktree exists, provide its direct `open` command; otherwise provide executable fetch/worktree/open commands; viewing recorded evidence does not require starting the application
* [ ] **BLOCKER when evidence belongs to a private repository:** Do not publish it through a public Pages site, public gist, or other public host without explicit user authorization; prefer authenticated PR presentation or a local review bundle
* [ ] **BLOCKER when a PR changes user-visible frontend:** The review surface shows the target prototype image and real implementation screenshot together for every acceptance-critical state, with matching state/viewport/theme/data and truthful labels; a non-visual frontend change carries an explicit waiver
* [ ] **BLOCKER when merge is used as acceptance:** Human-Confirmed remains open before merge; post-merge reconciliation verifies final-tree equivalence, records the merge audit fields, then checks acceptance, flips the banner, completes Final Reconciliation, and archives the PRD
...
* [ ] **BLOCKER:** Included a Realistic Validation Plan as a structured YAML oracle block parseable by deterministic tooling, with `id` / `behavior` / `reviewer` / `real_entry` / `expected` / `mock_boundary` / `tier` / `test_layer` / `required_for_acceptance` on every entry; every `reviewer: human` entry adds `presentation`; `R2`/`R3` entries add the provenance chain (`critical_value_source` / `must_cross` / `forbidden_bypasses` / `fresh_state_probe` / `final_tree_evidence`) and `R3` / human-confirm entries add `negative_control` + `expected_fail`; no `negative_control` was bought by adding a failure switch to production code
* [ ] Added low-fidelity prototype only when actually needed
* [ ] **BLOCKER when prototype files changed:** Loaded the repository's dedicated prototype skill when available; registered every new/updated independent prototype in the existing Prototype Hub (or created the minimal Hub when the delivery forms multiple independent prototypes), added a visible return path, preserved image provenance sidecars, and validated the Hub-to-prototype-to-Hub path on desktop and narrow width
