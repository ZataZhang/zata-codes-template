...
3. **互不相关的多个原型**：使用 prototype hub 统一登记。默认使用高密度目录并在右侧详情抽屉中预览；已有设计系统另有约定时服从既有约定。
...
新增或修改原型的最小 Hub 验证路径是：`Hub 卡片 → 原型默认态 → 一个关键结果态 → 返回 Hub`。桌面和窄屏各走一次；对纯图片卡片则验证 `Hub 卡片 → 原图` 与 `Hub 卡片 → 说明/旁车` 两条入口。Hub 只能从 registry 读取展示状态，失效探测或人工标记为 unavailable 的条目不得渲染成正常可用按钮。
...
- Hub 缩略图和产品视觉对照截图已隐藏 prototype chrome，交互演示截图如保留则明确标注；
