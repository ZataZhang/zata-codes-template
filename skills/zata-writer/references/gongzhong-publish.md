...
2. **构建前检查源 md**。两条路径（手动粘贴 / API 推送）都不会渲染 mermaid，来源组织也有约定，构建前先处理好：
   ...
   - **源 md 里不残留 mermaid 代码块**。粘贴路径下 pandoc 只会把它输出成普通代码块，读者看到的是源码；API 路径下脚本被剥、直接变纯文本。构建前先画成 PNG 流程图（画法参考 `gzh_build/make_flow_02.py`、`make_flow_04.py`），在 md 里替换为图片引用 + 图注，保证 md 和 HTML 渲染一致。
...
5. **视觉验证**。用无头 Chrome 截图检查，重点看标题装饰线、二级标题荧光底色、高亮句、表格表头、图片图注是否正常，流程图必须是渲染后的图片而非代码块，文末「参考资料」只有一份且与正文上标编号一一对应：
