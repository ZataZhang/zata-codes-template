...
本路线的唯一交付物是文章目录里的 `<文章名>_网页版.html`。构建零依赖（pandoc + python3 + 无头 Chrome 验证），**禁止**为网页版请求搭建任何工程化脚手架：不跑 npm/node，不建 React/Vite/TypeScript 工程，不产生 `package.json`、`node_modules`、tsconfig 等依赖文件；也不要改用需要装依赖或搭脚手架的其他 Skill（如 beautiful-article）。2026-09-10 曾因改走脚手架路线在用户项目里留下整套 node 工程文件，被要求把整个目录删除返工。
...
1. 不拷贝任何构建文件到文章目录。直接调用本 Skill 自带的 `assets/gzh_build/build_web.sh`（脚本自己定位同目录的样式与后处理脚本，可在任何工作目录运行）。
2. 文章 Markdown 保持普通结构即可：一级标题作文章标题，二级标题进目录，图片用相对路径引用 `image/` 下的本地文件。**不要在 md 里手写品牌动画 GIF**（`assets/brand/开头动画.gif`）——那是公众号版专属，公众号版的 `decorate.py` 会把它自动插到标题块下面，网页版的 `postprocess_web.py` 完全不碰它；网页版封面已有 kicker 和页脚落款，正文开头再来一遍是重复。
3. （可选）荧光高亮：在文章目录写 `gzh_build/<文章名>.highlights.sed`，每行一条 `s/要高亮的句子/<mark>要高亮的句子<\/mark>/`。句子必须逐字匹配正文。仅当确实需要高亮时才创建该目录。
4. （可选）文章专属封面：在文章目录写 `gzh_build/hero.html`，放一段内联 SVG（或其他 HTML）。后处理会把它装进封面区。SVG 建议用主题色系（米白底 #fbf7ee、锈红 #a6300e、灰线 #cfc8b6），画抽象示意而非复杂插画。仅当确实需要专属封面时才创建。
5. 构建：

   ```bash
   <本 Skill 目录>/assets/gzh_build/build_web.sh 文章.md "Zata 山外志 · 手记" "副标题，可省略"
   ```

   脚本内部：剥掉 H1（避免和封面重复）→ 应用 highlights → `pandoc -f markdown+mark -t html5 -s --toc --toc-depth=2 -H style_web.html` → `postprocess_web.py` 组装封面/目录/正文/资料来源/页脚 → 本地图片转 base64 内联。文章目录里新增的只有 `<文章名>_网页版.html`。
...
- **表格必须 `display: table` 覆盖 pandoc 默认样式**。pandoc 独立文档的默认 CSS 给 `table` 设了 `display: block`，文章里的 pipe 表格若源码较窄，pandoc 不会生成 colgroup 百分比列宽，表格会缩成内容宽（05 篇实测只占栏宽 55%）。style_web.html 的 table 规则里已加 `display: table;`，改主题时不要删。
