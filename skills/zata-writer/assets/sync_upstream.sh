...
#
# 2026-09-09 起本脚本不再是标准流程。标准流程改为：在模板仓库里直接改
# skills/zata-writer，提交后用仓库侧的 `scripts/sync_template.sh --skill zata-writer`
# 部署到各安装副本（见 SKILL.md「维护」一节）。本脚本降级为急救工具：仅在安装
# 副本被直接改过、需要把改动抢救回仓库时使用。它把无版本控制的副本单向覆盖进
# Git 仓库——下面的守卫能挡住文件删除，挡不住仓库已提交内容被旧副本盖回，用完
# 必须人工逐行复核 diff 再决定提交。
#
